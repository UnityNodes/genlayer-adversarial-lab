...
| F2 | The shipped default transform is literally `filter_text(args.prompt, {"NFKC","RmZeroWidth","NormalizeWS"})` + empty-prompt reject. The full built-in filter set is `TextFilter{NFC,NFKC,NormalizeWS,RmZeroWidth}` and `ImageFilter{Denoise,GaussianNoise,Unsharpen,JPEG}`. | Homoglyph / zero-width / whitespace tricks are neutralized out of the box. Paraphrase / retokenize / perplexity are **not** shipped filters - they must be written in Lua. |
